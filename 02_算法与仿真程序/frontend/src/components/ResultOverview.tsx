...
const isRateMetric = (key: string) => key.includes('利用率') || key.includes('稼动率');
...
const ResultOverview: React.FC<Props> = ({ data, prevData }) => {
  ...
  const objectiveText = algorithmName.includes('NSGA-II')
  ...
  // KPI 卡片用的指标数组
  const kpis = metricDefs.map((m) => ({ ...m, precision: 2 }));
  ...
  const comparisonCols = [
    ...
    ...metricDefs.map((m) => ({
      ...
      render: (_: unknown, row: any) => {
        ...
        const isUtil = isRateMetric(m.key);
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {kpis.map((kpi) => {
          ...
          const displayVal = isRateMetric(kpi.key) ? safeVal * 100 : safeVal;
