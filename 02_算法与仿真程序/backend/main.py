...
def _optimizer_display_name(cfg) -> str:
    ...
    if method == "dq_nsga2":
        return "NSGA-II（可选DQN种子，多目标自动匹配）"
...
async def get_run_detail(run_id: str):
    ...
    if balanced_dir.exists() and (balanced_dir / "summary.json").exists():
        ...
        if "NSGA-II" in (balanced_payload.get("algorithmName", stored_name) or stored_name):
...
async def reschedule(payload: dict):
    ...
    if history_entry is None:
        raise HTTPException(500, "运行历史记录丢失，无法确认原始数据")

    # 只允许使用当前运行绑定的 file_id；旧记录仅回退到自身 run_id。
    search_id = history_entry.get("file_id") or history_entry.get("id")
    candidates = [p for p in UPLOAD_DIR.glob(f"{search_id}_*") if "_speed_" not in p.name]

    if not candidates:
        raise HTTPException(500, "当前运行绑定的原始上传文件已丢失，请重新上传后运行")
    ...
    try:
        ...
        # 恢复原运行的配置口径；仅保留重调度的短迭代限制。
        cfg = ModelConfig()
        stored_params = history_entry.get("params") or {}
        for key, value in stored_params.items():
            ...
            try:
                ...
                raise HTTPException(500, f"历史参数 {key} 已损坏，请重新运行原任务")
            setattr(cfg, key, value)
        if stored_params:
            cfg._user_set_crane_overlap = True
        cfg = cfg.adapt_to_data(plates, parts, pp)
