...
def build_joint_schedule(
        ...
        for _, pr in plate_parts.iterrows():
            ...
            if pr["零件类型"] == "small":
                ...
                if is_n5 or need_grind:
                    ...
                    grind_dur += (
                        pp.small_grind_scan_minutes
                        if pp else cfg.small_grind_scan_minutes
                    )
    ...
    while events:
        ...
        if kind == 0:
            ...
            # 同一料框从首次进入工作站起只占用一个框位，最终转运时一次性
            # 记录完整区间。旧实现还记录每次坡口加工区间，导致同一料框
            # 被重复计数，峰值可能虚高到容量的 1.4～1.7 倍。
            bin_entry_time.setdefault(bin_key, arrival)
...
def capacity_violation_score(metrics: Dict[str, float]) -> float:
    """Return zero only when every reported buffer constraint is feasible.

    The simulator reports occupancies as ratios.  Keeping this check in the
    shared objective prevents SA+Tabu, GA+LNS and NSGA-II from selecting a
    faster schedule that exceeds a finite buffer or contains a deadlock.
    """
    ratio_keys = (
        "坡口缓存区峰值占用率",
        "半框缓存区峰值占用率",
    ...
    overflow = sum(
        max(0.0, float(metrics.get(key, 0.0) or 0.0) - 1.0)
        for key in ratio_keys
    )
    deadlocks = max(0.0, float(metrics.get("死锁警告数", 0.0) or 0.0))
    return overflow + deadlocks
...
def unified_capacity_objective(
    ...
    infeasible_penalty = 1000.0 * capacity_violation_score(metrics)
    return capacity_term + eps * secondary + infeasible_penalty
...
def select_best_by_capacity(
    ...
    feasible = [
        ...
        if capacity_violation_score(candidate[2]) <= 1e-9
    ]
    if feasible:
        candidates = feasible
...
def legacy_balanced_objective(
    ...
    infeasible_penalty = 1000.0 * capacity_violation_score(metrics)
    return base + idle_weight * idle_h + infeasible_penalty
